...
import { Command, TableIcon, BookmarkCheck, Bookmark, Plus } from "lucide-react"
...
const navMain = [
  { title: "All Resources", url: "#", icon: Command, isActive: false },
  { title: "Table", url: "#", icon: TableIcon, isActive: true }, // Set Table as active by default
]

const SKELETON_COUNT = 5

export function AppSidebar({ onResourceSelect, selectedResourceId, onViewChange, ...props }: AppSidebarProps) {
  const [activeItem, setActiveItem] = React.useState(navMain[1]) // Default to Table view
  ...
  const [sidebarWidth, setSidebarWidth] = React.useState<number>(280) // Wider sidebar
  ...
  // Notify parent of default view on mount
  React.useEffect(() => {
    onViewChange?.("table")
  }, [onViewChange])
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (storedWidth) setSidebarWidth(Number.parseInt(storedWidth, 10))
  ...
  return (
    <>
      <Sidebar collapsible="icon" className="flex-1 overflow-hidden [&>[data-sidebar=sidebar]]:flex-row" {...props}>
                ...
                <SidebarMenuButton asChild className="md:h-8 md:p-0">
                  <a href="#">
                    ...
                    <div className="grid flex-1 text-left text-sm leading-tight">
                      <span className="truncate font-semibold text-foreground">Resource Dir</span>
        ...
        <Sidebar collapsible="none" className={cn("hidden flex-1 md:flex", !open && "hidden")} ref={sidebarRef}>
                  ...
                  (loading ? (
                    ...
                    filteredResources.map((resource) =>
                      resource.id === selectedResourceId ? (
                        // Render selected resource larger
                        <div
                          key={resource.id}
                          className={cn(
                            ...
                            selectedResourceId === resource.id && "selected expanded-resource",
                        ...
                        >
                          <div className="flex w-full items-center gap-2 flex-wrap">
                            {resource.thumbnailUrl ? (
                              <img
                                src={resource.thumbnailUrl || "/placeholder.svg" || "/placeholder.svg"}
                                alt="Thumbnail"
                                className="w-24 h-24 object-cover rounded-lg" // Larger thumbnail
                            ...
                            <div className="flex flex-col">
                              <span className="break-words max-w-[300px] font-medium text-foreground text-lg">
                                {resource.videoTitle || resource.Title}
                              </span>
                              <Badge variant="outline" className="w-fit mt-1 text-foreground border-foreground/20">
                          ...
                          <span className="break-words max-w-[400px] text-sm text-muted-foreground">
